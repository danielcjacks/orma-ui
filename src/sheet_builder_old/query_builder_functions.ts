...
export const query_path_to_entity_name = (path_array: any, query: any, schema: OrmaSchema) => {
    let entity_name
    ...
    for (const i of range(0, path_array.length)) {
        ...
        const path_el_is_table_name = is_entity_name(path_el, schema)
...
export const get_nested_path_edge_tables = (
    ...
    const nested_path = safe_path_or(undefined, path_to_nested_path, query).filter(
        (el: string) => !!el
    )

    const path_to_any_object = dropLast(2, path_to_nested_path)

    const this_table_name = query_path_to_entity_name(path_to_any_object, query, schema)
    const edges =
        nested_path.length === 0
            ? get_edges(this_table_name, schema)
            : get_edges(last(nested_path), schema)
...
export const get_possible_edge_entity_names = (
    ...
    const this_entity_name = query_path_to_entity_name(path_to_subquery, query, schema)
...
export const get_select_fields = (path_to_select: any[], query: any, schema: any) => {
    ...
    const entity_name = query_path_to_entity_name(path_to_select, query, schema)
