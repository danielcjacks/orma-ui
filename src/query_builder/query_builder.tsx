...
const Subquery = observer(
  ({
    ...
    const possible_entities = get_all_edges(entity, orma_schema).map(
      (el) => el.to_entity
    )
...
const get_entity_resolvers = (entity_names: string[]) => {
  const entity_resolvers = entity_names.reduce<QueryBuilderResolvers>(
    (acc, entity) => {
      acc[entity] = {
        default: {},
...
const Field = observer(
  ({ parent, field }: { parent: Record<string, any>; field: string }) => {
...
const JsonObjectValue = observer(
  ({
    ...
    const DefaultRenderer: QueryBuilderResolvers[string]["renderer"] = (
      props
